...
export default function MyReports() {
  ...
  const openViewModal = (id) => {
    setViewError("");
    // Prevent stale snapshot flashing when switching between reports.
    setViewReport(null);
  ...
  const patchAttachmentsForReport = (id, attachments) => {
    ...
    if (String(activeViewReportId || "") === String(id)) {
      setViewReport((prev) => (prev ? { ...prev, attachments } : prev));
      setViewReportSnapshot((prev) => (prev ? { ...prev, attachments } : prev));
    }
  };
  ...
  const submitEdit = async (e) => {
    ...
    try {
      ...
      // If view modal is open for the same report, update its snapshot too.
      if (String(activeViewReportId || "") === String(editReportId || "")) {
        setViewReport(updated);
        setViewReportSnapshot(updated);
      }
  ...
  // Preserve attachment preview content during close animation (prevents a blank panel).
  useEffect(() => {
    if (imagePreview) {
  ...
  // Preload image thumbnails for the view modal so attachments show immediately.
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isViewModalOpen, viewReport, viewReportSnapshot, attachmentLoadStates]);
            ...
            <div className="account-approvals-detail-header-text">
              ...
              <div className="account-approvals-detail-subtitle">
                {(() => {
                  ...
                  return (
                    <span className="account-approvals-detail-name">
                      {summary || "Review the details, timeline, and attachments for this submission."}
                    </span>
                  );
                })()}
              </div>
                              ...
                              >
                                ...
                                {isImage && (loadState === "loading" || (!objectUrl && loadState !== "error")) ? (
                                  ...
                                  >
                                    ...
                                    {isImage ? (loadState === "error" ? "Retry preview" : "Load preview") : "Load"}
                                  </button>
                                )}
                                </div>
                                <div
      ...
      >
        {imagePreviewSnapshot ? (
          <>
            ...
            >
              <div
                className="myreports-attachment-media"
              >
                <img
